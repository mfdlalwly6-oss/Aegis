...
class DecisionRepository:
    ...
    def get_by_tx(self, tx_id: str) -> dict | None:
        return self.db.query_one(
            "SELECT * FROM decisions WHERE tx_id=? ORDER BY created_at DESC LIMIT 1",
            (tx_id,))
    ...
    def count_by_tenant(self, tenant_id: str) -> dict:
        rows = self.db.query(
            ...
            "FROM decisions WHERE tenant_id=? GROUP BY decision", (tenant_id,))
        ...
        return {"tenant_id": tenant_id, "total_decisions": total,
                "by_decision": by,
    ...
    def overview(self) -> dict:
        ...
        for r in rows:
            ...
            total += r["c"]
            total_risk += (r["avg_risk"] or 0) * r["c"]
        return {"total": total, "by_decision": by,
                "avg_risk": round(total_risk / max(total, 1), 4)}
    ...
    def mark_seen(self, idempotency_key: str, tenant_id: str, tx_id: str) -> bool:
        ...
        self.db.execute(
            ...
            "VALUES (?,?,?,?)", (idempotency_key, tenant_id, tx_id, utcnow()))
        return True
