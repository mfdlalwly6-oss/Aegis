...
class CaseRepository:
    ...
    def create(self, tenant_id: str, title: str, priority: str = "medium",
               narrative: str = "", tx_ids: list[str] | None = None,
        ...
        self.db.execute(
            ...
            "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (cid, tenant_id, title, "open", priority, narrative,
             json.dumps(tx_ids or []), json.dumps(alert_ids or []),
             json.dumps([]), now, now))
        return self.get(cid)
