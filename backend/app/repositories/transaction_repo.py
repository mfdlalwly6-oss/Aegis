...
class TransactionRepository:
    ...
    def create(self, row: dict, features: dict, raw: dict) -> dict:
        self.db.execute(
            ...
            (
                ...
                row.get("beneficiary_user_id"),
                row.get("beneficiary_country"),
                row.get("merchant_id"),
    ...
    def get(self, tx_id: str, tenant_id: str | None = None) -> dict | None:
        if tenant_id:
            return self.db.query_one(
                "SELECT * FROM transactions WHERE tx_id=? AND tenant_id=?", (tx_id, tenant_id)
            )
    ...
    def shared_device_accounts(self, tenant_id: str, device_id: str) -> list[str]:
        rows = self.db.query(
            "SELECT DISTINCT sender_account_id FROM transactions WHERE tenant_id=? AND device_id=?",
            (tenant_id, device_id),
        )
    ...
    def structuring_count(
        self, tenant_id: str, sender: str, low: float = 9000, high: float = 10000, days: int = 30
    ) -> int:
        rows = self.db.query(
