...
def _entry_hash(prev_hash: str, ts: str, tenant_id: str | None, actor: str,
                event_type: str, resource: str | None, resource_id: str | None,
                request_id: str | None, metadata_json: str) -> str:
    """SHA-256 over prev hash + canonical payload — insertion order defines the chain."""
    canonical = "|".join([prev_hash, ts, tenant_id or "", actor, event_type,
                          resource or "", resource_id or "", request_id or "", metadata_json])
...
class AuditRepository:
    ...
    def log(self, tenant_id: str | None, actor: str, event_type: str,
        ...
        entry = _entry_hash(prev, ts, tenant_id, actor, event_type, resource,
                            resource_id, request_id, meta_json)
        self.db.execute(
            "INSERT INTO audit_log (ts,tenant_id,actor,event_type,resource,"
            "resource_id,request_id,metadata_json,prev_hash,entry_hash) "
            "VALUES (?,?,?,?,?,?,?,?,?,?)",
            (ts, tenant_id, actor, event_type, resource, resource_id,
             request_id, meta_json, prev, entry))
    ...
    def verify_chain(self, limit: int = 10000) -> dict:
        ...
        for r in rows:
            if not r.get("entry_hash"):
                if chain_started:
                    return {"ok": False, "reason": "hash_gap_mid_chain", "row_id": r["id"], "checked": checked}
            ...
            expect = _entry_hash(prev, r["ts"], r["tenant_id"], r["actor"], r["event_type"],
                                 r["resource"], r["resource_id"], r["request_id"], r["metadata_json"])
        ...
        return {"ok": True, "checked": checked, "tip": prev,
                "legacy_skipped": legacy_skipped, "chain_started": chain_started}
