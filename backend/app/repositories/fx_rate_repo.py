...
class FxRateRepository:
    ...
    def add(self, base_ccy: str, quote_ccy: str, rate: float, *,
            rate_type: str = "mid", source: str = "aegis_reference",
        ...
        self.db.execute(
            ...
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
            (rid, base_ccy.upper(), quote_ccy.upper(), float(rate), rate_type,
             source, region, spread_pct, now, vf, valid_to, now))
        return self.get(rid)

    def get(self, rate_id: str) -> dict | None:
        return self.db.query_one("SELECT * FROM fx_rates WHERE rate_id=?", (rate_id,))

    def latest_valid(self, base_ccy: str, quote_ccy: str, *,
        ...
        regions = [region, "global"] if region and region != "global" else (
    ...
    def list_active(self, region: str | None = None) -> list[dict]:
        if region:
            return self.db.query(
                ...
                "ORDER BY fetched_at DESC", (region, utcnow()))
        return self.db.query(
            "SELECT * FROM fx_rates WHERE valid_to IS NULL OR valid_to>? "
            "ORDER BY fetched_at DESC", (utcnow(),))
