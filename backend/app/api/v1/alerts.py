...
def update_alert(alert_id: str, body: dict, request: Request,
                 owner=Depends(require_owner), registry=Depends(get_registry)):
    alert = registry.alerts.update_status(alert_id, body.get("status", "open"),
                                          assignee=body.get("assignee"))
    if not alert:
        raise HTTPException(404, "not_found")
    registry.audit.log(alert["tenant_id"], "owner", "alert.status_changed",
                       "alert", alert_id, getattr(request.state, "request_id", None),
