...
async def import_watchlist(tenant_id: str, file: UploadFile = File(...), owner=Depends(require_owner), registry=Depends(get_registry)):
    ...
    if len(data) > 2_000_000: raise HTTPException(413, "file_too_large")
    if not registry.tenants.get(tenant_id): raise HTTPException(404, "tenant_not_found")
    try: summary = import_csv(registry.watchlist_repo, tenant_id, data)
