...
def build_report_pdf(report: dict) -> bytes:
    ...
    doc = SimpleDocTemplate(buf, pagesize=A4, topMargin=14 * mm,
                            bottomMargin=14 * mm, leftMargin=14 * mm,
                            rightMargin=14 * mm,
    ...
    h1 = ParagraphStyle("h1", parent=styles["Title"], fontName=bold,
                        ...
                        textColor=colors.HexColor("#0F172A"), leading=22)
    sub = ParagraphStyle("sub", parent=styles["Normal"], fontName=base,
                         fontSize=9, alignment=TA_CENTER,
                         textColor=colors.HexColor("#64748B"), leading=13)
    h2 = ParagraphStyle("h2", parent=styles["Heading2"], fontName=bold,
                        fontSize=11, textColor=colors.HexColor("#0E7490"),
                        spaceBefore=10, spaceAfter=4)
    body = ParagraphStyle("body", parent=styles["Normal"], fontName=base,
    ...
    story.append(Paragraph(_ar(
        ...
        f"{report.get('tenant_country','-')} · خطة {report.get('tenant_plan','-')}"), sub))
    story.append(Paragraph(_ar(
        f"الفترة: {report.get('period_label','-')} | من {report.get('start_local','-')} "
        f"إلى {report.get('end_local','-')} | المنطقة الزمنية: {tz}"), sub))
    story.append(Paragraph(_ar(
        f"تاريخ الميلادي: {report.get('gregorian_date','-')} | الهجري: {hijri} | "
        f"التوليد: {gen_local}"), sub))
    ...
    band_lines = [[_ar("النطاق"), _ar("العدد")]] + \
        [[_ar(k), _fmt(v)] for k, v in bands.items()]
    ...
    ac_lines = [
        ...
        [_ar(""), _fmt(alerts.get("total")), _fmt(sum(
            v for k, v in alerts.get("by_status", {}).items()
            if k in ("open", "assigned", "in_review", "escalated")))],
        [_ar("الحالات"), _ar("إجمالي"), _ar("مفتوح")],
        [_ar(""), _fmt(cases.get("total")), _fmt(sum(
            v for k, v in cases.get("by_status", {}).items() if k != "closed"))],
    ]
    ...
    story.append(Paragraph(_ar(
        ...
        f"تجاوز SLA (+24 ساعة): {_fmt(mr.get('sla_breach_over_24h'))}"), body))
    ...
    story.append(Paragraph(_ar(
        ...
        "وتُعرض بالتوقيت المحلي للمؤسسة. التقرير لأغراض إدارة المخاطر الداخلية."), small))

    def _footer(canvas, _doc):
        canvas.saveState()
        canvas.setFont(base, 8)
        canvas.setFillColor(colors.HexColor("#94A3B8"))
        canvas.drawCentredString(A4[0] / 2, 8 * mm, _ar(
            f"AEGIS · صفحة {_doc.page} · وُلد {gen_local}"))
        canvas.restoreState()
