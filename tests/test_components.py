...
def _tx(**kw):
    base = {"tx_id": "t1", "tenant_id": "t", "amount": 100.0, "currency": "USD",
...
def test_rule_engine_evaluates_velocity():
    ...
    engine = RuleEngine([{
        "id": "T1", "name": "test", "severity": "high", "score": 0.5,
...
def test_rule_engine_disabled_rule():
    from app.rules.engine import RuleEngine
    engine = RuleEngine([{
        "id": "T2", "name": "off", "severity": "high", "score": 0.5,
        "enabled": False,
...
def test_graph_shared_device():
    g = GraphEngine()
    g.add_transaction(_tx(tx_id="x1", sender_account_id="acct_1",
                          device=DeviceContext(device_id="dev_a")))
    g.add_transaction(_tx(tx_id="x2", sender_account_id="acct_2",
                          device=DeviceContext(device_id="dev_a")))
    sig = g.score(_tx(tx_id="x3", sender_account_id="acct_3",
                      device=DeviceContext(device_id="dev_a")))
...
def test_behavior_score_in_orchestrator():
    ...
    o = DecisionOrchestrator(rules=None, ml=None, graph=None, aml_service=None,
                             features=None, transactions=None, decisions=None,
                             alerts=None, cases=None, audit=None, events=None,
                             notifications=None)
