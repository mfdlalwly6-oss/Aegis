...
class TestDecideThresholds:
    ...
    def test_weights_sum_to_one(self):
        total = (settings.WEIGHT_RULES + settings.WEIGHT_ML + settings.WEIGHT_GRAPH
                 + settings.WEIGHT_AML + settings.WEIGHT_BEHAVIOR)
...
class TestDecisionEngineE2E:
    ...
    def _tenant(self, client):
        ...
        r = client.post("/api/v1/admin/tenants", json={
            "name": f"DE-{uuid.uuid4().hex[:6]}", "type": "wallet", "country": "YE",
    ...
    def _post(self, client, tenant, body):
        ...
        return client.post("/api/v1/wallet/webhook", content=raw, headers={
            "Content-Type": "application/json", "x-api-key": tenant["api_key"],
    ...
    def test_clean_tx_allows(self, client):
        ...
        r = self._post(client, t, {"tx_id": "de-allow-1", "amount": 50, "currency": "USD",
    ...
    def test_unknown_currency_forces_review_not_allow(self, client):
        t = self._tenant(client)
        now = datetime.now(timezone.utc).isoformat()
        r = self._post(client, t, {"tx_id": "de-fxmiss-1", "amount": 500, "currency": "XXX",
                                   "sender_account_id": "a1", "beneficiary_account_id": "b1",
                                   "timestamp": now})
        assert r.status_code == 200
    ...
    def test_elevated_decision_creates_alert_and_case(self, client):
        t = self._tenant(client)
        now = datetime.now(timezone.utc).isoformat()
        r = self._post(client, t, {"tx_id": "de-alert-1", "amount": 500, "currency": "XXX",
                                   "sender_account_id": "a1", "beneficiary_account_id": "b1",
                                   "timestamp": now})
        assert r.status_code == 200
    ...
    def test_decision_trace_complete(self, client):
        t = self._tenant(client)
        now = datetime.now(timezone.utc).isoformat()
        r = self._post(client, t, {"tx_id": "de-trace-1", "amount": 75, "currency": "USD",
                                   "sender_account_id": "a1", "beneficiary_account_id": "b1",
                                   "timestamp": now})
        assert r.status_code == 200
        from tests.conftest import OWNER_HEADERS
        dr = client.get(f"/api/v1/admin/decisions/recent?limit=5", headers=OWNER_HEADERS)
        ...
        d = mine[0]
        for col in ("fx_proof_json", "tx_snapshot_json", "features_snapshot_json",
                    "rule_set_version", "model_version", "config_version"):
