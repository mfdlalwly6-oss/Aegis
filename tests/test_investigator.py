"""Investigator workflow tests — auth, queue, alerts, cases, RBAC."""
import json

from tests.conftest import create_tenant, sign


def _make_investigator(client, email="inv@aegis.local", password="inv-pass-2026"):
    r = client.post("/api/v1/admin/investigators", headers=client.owner_headers,
                    json={"email": email, "name": "محقق", "password": password})
...
def _send_review_tx(client, tenant):
    """Send a tx that lands in REVIEW (high amount + new device + impossible travel)."""
    payload = {
        "transaction": {
...
def test_investigator_login_and_me(client):
    inv = _make_investigator(client)
    token = _login(client)
    r = client.get("/api/v1/investigator/me", headers=_inv_headers(token))
    assert r.status_code == 200
    assert r.json()["email"] == "inv@aegis.local"
...
def test_merchant_token_cannot_access_investigator_routes(client):
    from tests.conftest import merchant_token
...
def test_review_queue_and_alert_lifecycle(client):
    ...
    # detail includes transaction + history
...
def test_case_lifecycle_and_fraud_marking(client):
    ...
    # graph now knows the fraud account
...
def test_investigator_deactivation_blocks_login(client):
    inv = _make_investigator(client)
    client.delete(f"/api/v1/admin/investigators/{inv['investigator_id']}",
